...
export default function EditPropertyPage() {
  ...
  const [images, setImages] = useState<{ file?: File; preview: string, publicId?: string }[]>([])
  ...
  useEffect(() => {
    const fetchProperty = async () => {
        try {
            ...
            setPaymentPeriod(data.paymentPeriod || "")
            ...
            if (data.images) {
                const initialImages = data.images.map((img: any) => ({
                    preview: img.url,
                    publicId: img.publicId, // If we want to keep them on backend
                }))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const existingImages = images.filter(img => !img.file).map(img => ({ url: img.preview, publicId: img.publicId as string }))
      
      if (newFiles.length > 0) {
        toast.loading("Uploading new images...", { id: "upload" })
        const newlyUploadedUrls = await propertyService.uploadImages(newFiles)
        uploadedImagesUrls = [...existingImages, ...newlyUploadedUrls]
      ...
      toast.success("Property updated successfully!", { id: "update" })
  ...
  return (
    <div className="p-6 lg:p-8 max-w-4xl mx-auto">
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
        ...
        <h1 className="font-heading text-2xl lg:text-3xl font-bold text-foreground">Edit Property</h1>
        <p className="text-muted-foreground mt-1">Update the details below to modify your listing</p>
      </motion.div>
                    ...
                    <div key={index} className="relative aspect-square rounded-lg overflow-hidden bg-muted">
                      ...
                      <Button
                        ...
                        onClick={async (e) => {
                          ...
                          if (image.publicId) {
                            try {
                              toast.loading("Deleting image...", { id: "delete-img" })
                              await propertyService.deleteImage(image.publicId)
                              toast.success("Image deleted", { id: "delete-img" })
                            } catch (err) {
                              toast.error("Failed to delete image on server", { id: "delete-img" })
                              return // Prevent UI removal if server failed
                            }
                          }
